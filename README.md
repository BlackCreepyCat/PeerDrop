...
PeerDrop uses PeerJS (WebRTC) to open a direct data channel between two browsers. Files never touch a server: they stream straight from sender to receiver, chunk by chunk, over an encrypted WebRTC connection.
...
Open PeerDrop and choose Server.
A 6-character connection code is generated automatically.
Drag and drop files onto the server screen (or click to browse) to add them to the shared library. Files can be added or removed at any time — connected receivers are notified instantly.
Share the 6-character code with whoever should receive the files.
As receivers connect, they appear in the Receivers panel with live download progress and per-file ACK (acknowledgment) status, confirming exactly which files each receiver has fully downloaded.
Multiple receivers can connect and download at the same time, each choosing their own subset of files.
Receive mode

The peer who wants to download files.

Open PeerDrop and choose Receive.
Enter the 6-character code given by the server and click Connect.
(Optional, recommended) Choose a destination folder. If the browser supports it, files save directly to that folder with no per-file save dialog. If no folder is chosen, each file triggers a standard browser download instead.
Browse the list of files the server is hosting, check the ones you want, and click Download selected. Files already fully present in the chosen folder are marked and can be skipped automatically.
If the connection drops during a transfer, PeerDrop reconnects automatically and resumes the file exactly where it left off.
Getting started

PeerDrop is a single HTML file with no build step and no server-side component required for the file transfer itself.

Download p2ptransfer.html (or clone this repository).
Open the file directly in a browser, or serve it from any static web host / local web server.
Share the URL (or the file) with the person on the other end — one of you picks Server, the other picks Receive.

Note: the file loads the PeerJS library from a CDN (by default https://unpkg.com/peerjs@1.5.4/dist/peerjs.min.js — see PeerJS for other CDN options, e.g. jsDelivr). An internet connection is required for the initial connection handshake between peers, even though the actual file transfer afterward is direct peer-to-peer.
...
Technical notes
Transport: WebRTC DataChannel, via PeerJS with binary serialization.
